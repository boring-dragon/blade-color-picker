...
<div x-data='{  isOpen: false , colors: @json($colors), colorSelected: @json($colorSelected) }' x-cloak>
    <div class="mb-5">
        <div class="flex items-center">
            <div>
                @if ($label)
                    <label for="{{ $id }}" class="block font-bold mb-1"> {{ $label }}</label>
                @endif
                <input @if ($id) id="{{ $id }}" @endif
                    @if ($name) name="{{ $name }}" @endif
                type="text" placeholder="{{ $placeholder }}"
                class="border border-transparent shadow px-4 py-2 leading-normal text-gray-700 bg-white rounded-md
                                ...
                                <div class="px-2">
                                    ...
                                    <template x-if="colorSelected != color">
                                        <div @click="colorSelected = color" @keydown.enter="colorSelected = color"
                                            role="checkbox" tabindex="0" :aria-checked="colorSelected"
